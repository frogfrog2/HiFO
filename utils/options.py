...
def parse(opt_path, root_path, is_train=True):
    ...
    Args:
        ...
        index (int): Index of the current experiment.
...
def parse_indi(opt_path, root_path, index, is_train=True):
    """Parse option file.
    ...
    if is_train:  # specify training log paths
        experiments_root = osp.join(root_path, 'experiments', opt['name']) + f'_Pair{index}'
        opt['path']['experiments_root'] = experiments_root
        opt['path']['models'] = osp.join(experiments_root, 'models')
        opt['path']['log'] = osp.join(experiments_root, 'log')
        opt['path']['visualization'] = osp.join(experiments_root, 'visualization')
    else:  # specify test log paths
        results_root = osp.join(root_path, 'results', opt['name']) + f'_Pair{index}'
        opt['path']['results_root'] = results_root
        opt['path']['log'] = osp.join(results_root, 'log')
        opt['path']['visualization'] = osp.join(results_root, 'visualization')
...
def parse_options_init(root_path, is_train=True):
    parser = argparse.ArgumentParser()
    parser.add_argument('--opt', type=str, required=True, help='Path to option YAML file.')
...
def parse_options_indi(root_path, index, is_train=True):
    parser = argparse.ArgumentParser()
    parser.add_argument('--opt', type=str, required=True, help='Path to option YAML file.')

    args = parser.parse_args()
    opt = parse_indi(args.opt, root_path, index, is_train=is_train)
