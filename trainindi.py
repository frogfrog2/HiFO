...
from utils import (AvgTimer, MessageLogger, get_env_info, get_root_logger,
                   init_tb_logger)
from utils.options import dict2str, parse_options_init, parse_options_indi
...
def train_pipeline(root_path, pair_index):
    bestloss = 1000000.0
    # parse options, set distributed setting, set random seed
    opt = parse_options_indi(root_path, pair_index, is_train=True)
    opt['root_path'] = root_path

    opt['datasets']['train_dataset']['pair_index'] = pair_index
    opt['datasets']['train_dataset']['template_index'] = 30
    opt['datasets']['test_dataset']['pair_index'] = pair_index
    opt['datasets']['test_dataset']['template_index'] = 30
        ...
        while model.curr_epoch < total_epochs:
            ...
            for train_data in train_loader:
                ...
                if total_loss < bestloss:
                    ...
                    #print(f"Best model saved at iter {model.curr_iter} with loss {bestloss:.4f}")
...
if __name__ == '__main__':
    ...
    opt = parse_options_init(root_path, is_train=True)
    start = opt['datasets']['train_dataset']['start_index']
    end = opt['datasets']['train_dataset']['end_index']
    for i in range(start, end):
        print(f"Training on pair {i}...")
        train_pipeline(root_path, i)
