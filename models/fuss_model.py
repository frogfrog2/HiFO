...
class FussModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        #if 'cluster_loss' in self.losses:
        #    if not hasattr(self, "curr_epoch"):
        #        self.compute_cluster_loss(Pxy.squeeze(0), Pyx.squeeze(0), data_x['faces'].squeeze(0), data_y['faces'].squeeze(0), None)
        #    else:
        #        self.compute_cluster_loss(Pxy.squeeze(0), Pyx.squeeze(0), data_x['faces'].squeeze(0), data_y['faces'].squeeze(0), self.curr_epoch)
        ...
        #self.loss_metrics = dict() ###### fine-tune
    ...
    def compute_shape_interpolation_chamfer_loss(self, vert_x, vert_x_pred_arr, vert_y, vert_y_pred_arr, face_x, face_y):
        if 'chamfer_shape_loss' in self.losses:
            ...
            #if not hasattr(self, "curr_epoch"):
            #    wei = 10.0
            #elif self.curr_epoch<=100:
            #    wei = 1.0
            #else:
            #    wei = 10.0

            wei = 1.0 # 10.0
            self.loss_metrics['l_cd'] += wei * self.compute_chamfer_distance(vert_x, vert_y_pred_arr[:, :, self.pose_timestep],
                                                                          face_x, face_y)

            self.loss_metrics['l_cd'] += wei * self.compute_chamfer_distance(vert_y, vert_x_pred_arr[:, :, self.pose_timestep],
                                                                      face_y, face_x)

    def compute_chamfer_distance(self, vert_x, vert_y, face_x, face_y):
        #if not torch.isfinite(vert_x).all():
        #    print("NaN or Inf!")
        #    print("Verts Max:", vert_x.max().item())
        #    print("Verts Min:", vert_x.min().item())
    
        #if not torch.isfinite(vert_y).all():
        #    print("NaN or Inf!")
        #    print("Verts Max:", vert_y.max().item())
        #    print("Verts Min:", vert_y.min().item())

        mesh_x = Meshes(verts=[vert_x], faces=[face_x])
        mesh_y = Meshes(verts=[vert_y], faces=[face_y])

        sample_x = sample_points_from_meshes(mesh_x, 20000) # 20000 for pretraining, 100000 for individual training
    ...
    def compute_permutation_matrix(self, feat_x, feat_y, bidirectional=False):
        ...
        #if not hasattr(self, "curr_epoch"):
        #    tau = 0.006145404664
        #elif self.curr_epoch<3:
        #    tau = 0.07
        #elif self.curr_epoch>8:
        #    tau = 0.006145404664
        #else:
        #    tau = 0.07 / (1.5 ** (self.curr_epoch - 2))

        #if not hasattr(self, "curr_epoch"):
        #    tau = 0.006145404664
        #elif self.curr_epoch<=40:
        #    tau = 0.07
        #elif self.curr_epoch>140:
        #    tau = 0.006145404664
        #else:
        #    tau = 0.07 / (1.5 ** (((self.curr_epoch-1) // 20) - 1))

        #tau = 0.006145404664
        tau = 0.07
        Pxy = self.networks['permutation'](similarity, tau)

        if bidirectional:
            Pyx = self.networks['permutation'](similarity.transpose(1, 2), tau)
            return Pxy, Pyx
        else:
            return Pxy
    ...
    def compute_cluster_loss(self, Pxy, Pyx, face_x, face_y, epo):
        self.loss_metrics['l_cluster'] = self.losses['cluster_loss'](Pxy, Pyx, face_x, face_y, epo)
    ...
    def deform_template(self, data):
        ...
        Ptx = Ptx.squeeze(0)
        Pxt = self.compute_permutation_matrix(feat_x, feat_t, bidirectional=False)
        Pxt = Pxt.squeeze(0)

        output_path = os.path.join(self.opt['path']['results_root'], 'visualization')

        #Ptxcpu = np.array(Ptx.cpu())
        #np.save(os.path.join(output_path, f"Ptxsoft_{name_x}.npy"), Ptxcpu)
        #Pxtcpu = np.array(Pxt.cpu())
        #np.save(os.path.join(output_path, f"Pxtsoft_{name_x}.npy"), Pxtcpu)

        ##### like FUSS paper
        vert_x_pred_arr = self.compute_displacement(vert_t, vert_x, face_t, Ptx)
        #Ptxcpu = np.array(Ptx.cpu())
        #np.save(os.path.join(output_path, f"ptx_{name_x}.npy"), Ptxcpu)

        ##### original (calculate p2p_tx)
        #Cxt = evecs_trans_t @ (Ptx @ evecs_x)
        #p2p_tx = fmap2pointmap(Cxt, evecs_x, evecs_t)

        #Ctx = evecs_trans_x @ (Pxt @ evecs_t)
        #p2p_xt = fmap2pointmap(Ctx, evecs_t, evecs_x)
        #p2pxt = np.array(p2p_xt.cpu())
        ##np.save(os.path.join(output_path, f"p2pxt_{name_x}.npy"), p2pxt)
        #vert_t_pred_arr = self.compute_displacement(vert_x, vert_t, face_x, None, p2p_xt)
        #deformed_t = vert_t_pred_arr[:, :, self.pose_timestep] * data_t['face_area']
        ##np.save(os.path.join(output_path, f"displt_{name_x}.npy"), np.array(deformed_t.cpu()))
        
        #p2p = np.array(p2p_tx.cpu())
        ##np.save(os.path.join(output_path, f"p2p_{name_x}.npy"), p2p)
        ...
        np.save(os.path.join(output_path, f"displx_{name_x}.npy"), np.array(deformed_verts.cpu()))
