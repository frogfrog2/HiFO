...
class BaseModel:
    ...
    @torch.no_grad()
    def build_ssm(self, dataloader_reference, dataloader_train, dataloader_test, ind=None):
        self.eval()
        logger = get_root_logger()
        ...
        self.get_reference_shape(dataloader_reference, '19') # template_name # '243'
        ...
        output_path = os.path.join(self.opt['path']['results_root'], 'visualization') #"./results/fuss_pancreas/visualization"
        deformed_training_shapes = torch.empty(0, n_vertices, 3).to(self.device)
        pbar = tqdm(dataloader_train)
        if ind is None:
            for index, data in enumerate(pbar):
                nam = data['name'][0]
                np.save(os.path.join(output_path, f"verts_{nam}.npy"), data['verts'])
                np.save(os.path.join(output_path, f"faces_{nam}.npy"), data['faces'])
                np.save(os.path.join(output_path, f"area_{nam}.npy"), data['face_area'])
        ...
        #dts = np.array(deformed_training_shapes.cpu())
        #np.save(os.path.join(output_path, "deformed_training_shapes.npy"), dts)
        #print(f"Deformed training shapes saved to {output_path}/deformed_training_shapes.npy")
        np.save(os.path.join(output_path, "verts_temp.npy"), self.template['verts'])
        np.save(os.path.join(output_path, "faces_temp.npy"), self.template['faces'])
        np.save(os.path.join(output_path, "area_temp.npy"), self.template['face_area'])
    ...
    def update_template(self, item, name, verts, faces, num_evecs, csv_path):
        ...
        mesh_info = pd.read_csv(csv_path, dtype={"file_name": str})
    ...
    def _init_training_setting(self):
        ...
        ##### fine-tuning
        #for p in self.networks['feature_extractor'].parameters():
