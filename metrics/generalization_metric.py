...
def calculate_generalization(ssm_model, dataloader_test, deformed_testing_shapes, logger, device, output_path, template):
    ...
    for mode in tqdm(range(1, ssm_model.variances.shape[0] + 1)):
        ...
        for index, test_data in enumerate(dataloader_test):
            ...
            #np.save(os.path.join(output_path, f"oriverts_{index}.npy"), original_verts.cpu().numpy())
            #np.save(os.path.join(output_path, f"orifaces_{index}.npy"), test_data['faces'].cpu().numpy())
            #np.save(os.path.join(output_path, f"reconverts_{index}.npy"), recon_deformed_shape.cpu().numpy())
            #np.save(os.path.join(output_path, f"reconfaces_{index}.npy"), template['faces'].cpu().numpy())
