...
from losses.reg import criterion
...
def train_pipeline(root_path):
    ...
    exppath = opt['path']['experiments_root']
    ...
    try:
        while model.curr_epoch < total_epochs:
            ...
            for train_data in train_loader:
                ...
                if model.curr_epoch == total_epochs:
                    #if not chk:
                    #    firstpair = copy.deepcopy(train_data)
                    #    chk = True
                    losses = model.get_loss_metrics()
                    total_loss = losses['l_total']
                    if total_loss < bestloss:
                        bestloss = total_loss
                        model.save_model(net_only=True, best=False)
                    #model.eval()
                    #with torch.no_grad():
                    #    vertx1, verty1 = model.deform_single(firstpair)
                    #    #print(vertx1.shape, verty1.shape)
                    #    xlist.append(vertx1.detach().cpu().numpy())
                    #    ylist.append(verty1.detach().cpu().numpy())
                    #model.train()
        ...
        #xnp = np.array(xlist)
        #ynp = np.array(ylist)
        #np.save(f'{exppath}/visualization/xnp.npy', xnp)
        #np.save(f'{exppath}/visualization/ynp.npy', ynp)
        #print(firstpair['first']['name'], firstpair['second']['name'])
        #firstvx = firstpair['first']['verts']
        #firstvy = firstpair['second']['verts']
        #firstfx = firstpair['first']['faces']
        #firstfy = firstpair['second']['faces']
        #np.save(f'{exppath}/visualization/firstvx.npy', firstvx.cpu().numpy())
        #np.save(f'{exppath}/visualization/firstvy.npy', firstvy.cpu().numpy())
        #np.save(f'{exppath}/visualization/firstfx.npy', firstfx.cpu().numpy())
        #np.save(f'{exppath}/visualization/firstfy.npy', firstfy.cpu().numpy())
